...
class TranslateMod(loader.Module):
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig("DEFAULT_LANG", "en", "Language to translate to by default",
                                          "API_KEY", "trnsl.1.1.20200110T192644Z.b5fdbe36a0ae2e95.020228c77cd8eb9b0279d9c37377bf1b22658a16", "API key from https://translate.yandex.com/developers/keys")
    ...
    async def translatecmd(self, message):
        ...
        if len(text) == 0:
            await message.edit(_("Ungültiger Text"))
            return
        if args[0] == "":
        ...
        ret = _("<b>Ich habe </b><code>{text}</code>\n<b>von </b><code>{frlang}</code><b> nach </b>"
                + "<code>{to}</code><b> übersetzt. da: </b>\n<code>{output}</code>")
