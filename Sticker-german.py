...
class StickersMod(loader.Module):
    ...
    async def kangcmd(self, message):  # noqa: C901 # TODO: reduce complexity a LOT
        ...
        if len(args) != 1 and len(args) != 2:
            ...
            await message.edit(_("Gib' einen Namen ein, und gebe optional noch einen Emaji hinzu!"))
            return
        ...
        if not (sticker.sticker or sticker.photo):
            await message.edit(_("Antworte auf ein Foto/ GIF, um ihm einen Namen zu vergeben."))
            return
        ...
        try:
            ...
            if animated:
                async with self._lock:
                    ...
                    async with conv:
                        ...
                        if buttons is not None:
                            ...
                            await message.edit("Etwas ist schiefgelaufen")
                            return
                        ...
                        if ".PSD" in r0.message:
                            ...
                            logger.error(r0)
                            await message.edit(_("<code>Dies ist kein animiertes Stickerpaket.</code>"))
                            msgs = []
                        ...
                        if "/done" not in r2.message:
                            ...
                            logger.error(r1)
                            logger.error(r2)
                            await message.edit(_("<code>Leider ist etwas intern schiefgelaufen!</code>"))
                            return
                    msgs = []
                ...
                if "emoji" in r2.message:
                    ...
                    logger.error(r2)
                    await message.edit(_("<code>Bitte verwende normale Emoji(s).</code>"))
                    return
                    ...
                    async with self._lock:
                        ...
                        async with conv:
                            ...
                            if buttons is not None:
                                ...
                                await message.edit("<code>Bitte erstelle zuerst ein Paket!</code>")
                                return
                            else:
                                logger.warning("there's no buttons!")
                                m0 = await message.client.send_message("t.me/" + self.config["STICKERS_USERNAME"],
                                                                       "/cancel")
                                await message.edit("<code>Etwas ist schief gelaufen</code>")
                                return
                            # We have sent the pack we wish to modify.
                            # Upload sticker
                            r0 = await conv.get_response()
                            if ".TGS" in r0.message:
                                logger.error("bad response from stickerbot 0")
                                logger.error(r0)
                                await message.edit(_("<code>Dies ist ein animiertes Paket</code>"))
                                msgs = []
                                async for msg in message.client.iter_messages(entity="t.me/"
                                                                              + self.config["STICKERS_USERNAME"],
                                                                              min_id=first.id,
                            ...
                            if "120" in r0.message:
                                logger.error("bad response from stickerbot 0")
                                logger.error(r0)
                                await message.edit(_("<code>Dieses Stickerpaket ist voll. Lösche ein paar Sticker, oder versuche, ein neues Pack "
                                                     "zu erstellen.</code>"))
                                msgs = []
                                async for msg in message.client.iter_messages(entity="t.me/"
                                                                              + self.config["STICKERS_USERNAME"],
                            ...
                            if "/done" in r2.message:
                                ...
                                logger.error(r2)
                                logger.error("Bad response from StickerBot 0")
                                await message.edit(_("<code>Leider ist intern etwas schiefgelaufen!</code>"))
                            await message.client.send_read_acknowledge(conv.chat_id)
                            if "/done" not in r2.message:
                                # That's an error
                                logger.error("Bad response from StickerBot 1")
                        ...
                        if "emoji" in r2.message:
                            ...
                            logger.error("Bad response from StickerBot 2")
                            logger.error(r2)
                            await message.edit(_("<code>Bitte wähle normale Emoji(s).</code>"))
                            return
                finally:
                    thumb.close()
        finally:
        ...
        await message.edit(_("<code>Sticker hinzugefügt zum</code> <a href='{}'>Pack</a><code>!</code>").format(packurl))
    ...
    async def gififycmd(self, message):
        ...
        if target is None or target.file is None or target.file.mime_type != "application/x-tgsticker":
            await utils.answer(message, _("<code>Wähle bitte einen animierten Sticker, um ihn in ein GIF umzuwandeln.</code>"))
            return
        try:
            file = BytesIO()
